...
class CNN_classification1(chainer.Chain):
    ...
    def forward(self, x):
        ...
        hi = self.conv(img)
        print "conv1"+str((hi.data).shape)
        hi = self.conv(hi)
        print "conv2"+str((hi.data).shape)
        hi = F.max_pooling_2d(hi,2,stride = 2)
        print "pool1"+str((hi.data).shape)
        hi = self.conv(hi)
        print "conv3"+str((hi.data).shape)
        hi = F.max_pooling_2d(hi,2,stride = 2)
        print "pool2"+str((hi.data).shape)
        hi = F.relu(self.li1(hi))
        print "full_con1"+str((hi.data).shape)
