...
def load_picture(data_label,scale):
    ...
    img_array = np.reshape(img_array,(1,img_shape[2]*img_shape[1]*img_shape[0]))
    ...
    for i in range(min_directly_n,max_directly_n+1):
        for j in range(max_picture_n+1):
            for neg_pos in range(2):
                ...
                img_data = load_picture(data_label,scale)
                xy_data = load_rectangle(data_label,neg_pos,scale)

                xy = np.reshape(xy_data[0],(1,8))

                #imgg = [img_data(i) for i in range(57600)]
                #rec = [xy(i) for i in range(8)]
                #aaa = zip(img_data,xy)
                #aaa = np.array(aaa)
                #print aaa.shape

                #a = img_data + xy_data[0]

                for k in range(len(xy_data)):
                    #X.append(np.array([img_data,xy_data[k]]))
...
if __name__ == '__main__':
    ...
    print Xt[0][0].shape
    #Xt = np.reshape(Xt,(len(Xt),Xt[0][0].shape[0]+Xt[0][1].shape[0]))
